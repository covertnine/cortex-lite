...
<section id="section-single" class="content-single content-none">
	<header class="entry-header entry-header-page mar20B<?php if ( ! empty( $featured_header ) ) { echo ' dark-overlay';} ?>">
		<?php if ( has_post_thumbnail() ) { ?>
		<figure class="entry-image" data-center="background-position: 0px 0px;" data-top-bottom="background-position: 0px -100px;" data-anchor-target=".entry-header" <?php if ( ! empty( $featured_header ) ) { echo 'style="background: url('.esc_url( $featured_header[0] ).') center center no-repeat; background-size: cover; background-position: center center;"'; } ?>></figure>
		<?php } ?>
	...
	<div class="container">
		...
		<?php elseif ( is_search() ) : ?>
			...
			<div class="mar50B">
				<p><?php _e( 'Sorry, but nothing matched your search terms. Please try again with some different keywords.', 'cortex' ); ?></p>
				<?php get_search_form(); ?>
			</div>
		...
		<?php else : ?>
			...
			<div class="mar50B">
				<p><?php _e( 'It seems we can&rsquo;t find what you&rsquo;re looking for. Perhaps searching can help.', 'cortex' ); ?></p>
				<?php get_search_form(); ?>
			</div>
