...
$cortex_counter++; ?>" class="simple_slider<?php if ( $cortex_customClass != '' ) { echo ' '.$cortex_customClass; } ?>">
		...
		while ( have_rows( 'slides' ) ) {
			...
			$cortex_link     = esc_url( get_sub_field( 'link' ) );
			$cortex_external_link = esc_url( get_sub_field( 'external_link' ) );
	...
	?>
			...
			<li class="slide img_slide_container">
				...
				<?php if ( ( ! empty( $cortex_link ) ) || ( ! empty( $cortex_external_link ) ) ) { ?>
					<a href="<?php if ( ! empty( $cortex_external_link ) ) { echo $cortex_external_link;
} else { echo $cortex_link; } ?>"<?php if ( ! empty( $cortex_external_link ) ) { echo ' target="_blank"'; }?>>
				<?php } ?>
